...
inline std::ostream& o(loglevel _level)
{
	if (_level == loglevel::ll_error || _level == loglevel::ll_warn)
	{
		return	std::cerr;
	}
	return	std::cout;
...
void log(loglevel _level, char const* _format, ...)
{
	if (_level <= sLoglevel)
	{
		...
		vsnprintf(buff.data(), buff.size(), _format, args);
        va_end(args);
		o(_level) << buff.data() << std::endl;
	}
}
...
void progress_inc_string(std::string_view _line, size_t _inc)
{
	...
	if (!_line.empty())
	{
		...
		if (!_line.empty())
		{
			o(loglevel::ll_info) << _line.substr(0, 30) << std::flush;
		}
	}
...
void progress_end()
{
	...
	progress_draw(true);
	o(loglevel::ll_info) << std::flush << std::endl;
	sMode	= 0;
}
...
void progress_draw(bool _force)
{
	...
	if (sProgress.cache != r)
	{
		...
		o(loglevel::ll_info) << "\r" << sProgress.cache << clear_line << std::flush;
	}
