...
static thread_local	assembler*	gCurrent	= nullptr;
...
assembler::assembler(size_t _stacksize, size_t _stackdepth)
...
assembler* assembler::current()
{
	return	gCurrent;
}


void assembler::add_memory(void* m, size_t s)
{
	mMemory.add({m, (void*)(((intptr_t)m) + s)});
}


bool assembler::remove_memory(void* m)
{
	return	!mMemory.release(m).empty();
}


bool assembler::in_memory(void* m, size_t s)
{
...
void assembler::local_alloc(vdatatype _datatype)
{
...
auto assembler::local(size_t _index) -> void*
{
...
void assembler::run(vmvariable& _retval, vdecl_function* _func, view<vmreg> _arguments)
{
	vmkepp		keep(this);
	auto	a	= mArguments;
...
void assembler::run(vmvariable& _retval, pvlnode& _node)
{
	vmkepp		keep(this);
#if	0
