...
{
public:
	static constexpr double	moment_speed	= 0.90f;
	...
	void set_axis(uint32_t _axis_mask)
	{
		mAxis	= _axis_mask;
	}
	...
	void release()
	{
		this->mScrollMoment	*= 1.0f / mLastDelta;	// 秒あたりの移動量
		this->mPressed		= false;
	...
	template<class NodeT>
	constexpr void input(NodeT& _node, input_event const* _e, vec2<float>* _pos = nullptr) noexcept
	{
		vec2<float>	pos		= _pos
							? *_pos
	...
	bool update(float _delta)
	{
		...
		if (!mPressed)
		{
			mScroll.x	+= mScrollMoment.x * _delta;
			mScroll.y	+= mScrollMoment.y * _delta;

			mScrollMoment.x = (float)((float)mScrollMoment.x - (mScrollMoment.x * moment_speed * _delta));
			mScrollMoment.y = (float)((float)mScrollMoment.y - (mScrollMoment.y * moment_speed * _delta));
	...
	//! 範囲外にはみ出している量を取得します。
	//! 最小値より小さい場合は負の数、最大値より大きい場合は正の数を返します。
	vec2<float> get_overfllow() const noexcept
	{
		vec2<float>	r(0, 0);
		for (int i = 0; i < mScrollRange.col_size; ++i)
		{
			r[i]	= (float)mScrollRange[i].out_of(mScroll[i]);
		}
		return	r;
	}
