...
{
public:
	using	super_type	= updatable_node<pager_node<NodeT>>;
...
protected:
	...
	virtual void on_update(float _delta) override
	{
		...
		if (mScrollPrev != now)
		{
			...
			if (pi == super_type::npage)
			{
				s	= {0, 0, 0};
			}
			s.x	= compute_scroll_position(s.x + mScrollPrev.x, { -(float)(super_type::children_count() - 1) * this->size().width , 0 });
			s.y	= compute_scroll_position(s.y + mScrollPrev.y, { -(float)(super_type::children_count() - 1) * this->size().height, 0 });
