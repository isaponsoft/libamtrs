...
struct	code_buffer
{
	...
	bool add_load(const vmstring& _type)
	{
		...
		{
AMTRS_DEBUG_LOG("Unkown type %s", vmstring(_type).c_str());
			return	false;
		}
		++mStackPoint;
		asmcode*	ac	= this->createBlankCode();
...
AMTRS_DEBUG_LOG("Unkown type %s", vmstring(_type).c_str());
	...
	bool add_load(vmclass* _clz)
	{
		...
		{
AMTRS_DEBUG_LOG("Unkown type %s", vmstring(type).c_str());
			return	false;
		}
		++mStackPoint;
		asmcode*	ac	= this->createBlankCode();
	...
	void add_inc(vmclass* _clz)
	{
		...
		{
			AMTRS_DEBUG_LOG("inc can't not pointer NG");
			opecode = opecode_id::LOAD_REF;
		}
		...
		{
			AMTRS_DEBUG_LOG("inc can't not float NG");
			switch (size)
			{
				case 4 :	opecode = opecode_id::LOAD_FLOAT;	break;
				case 8 :	opecode = opecode_id::LOAD_DOUBLE;	break;
		...
		{
			AMTRS_DEBUG_LOG("inc can't unkown type %s NG", _clz->name().c_str());
			return;
		}
	...
	void add_dec(vmclass* _clz)
	{
		...
		if (passion_is_gcpointerable(_clz->name()))
		{
			AMTRS_DEBUG_LOG("dec can't not pointer NG");
			opecode = opecode_id::LOAD_REF;
		}
		else if (passion_size_is_addressable(_clz->name()))
		{
		...
		{
			AMTRS_DEBUG_LOG("dec can't not float NG");
			switch (size)
			{
				case 4 :	opecode = opecode_id::LOAD_FLOAT;	break;
				case 8 :	opecode = opecode_id::LOAD_DOUBLE;	break;
		...
		{
			AMTRS_DEBUG_LOG("dec can't unkown type %s NG", _clz->name().c_str());
			return;
		}
	...
	void add_lea_array(vmclass* _clz)
	{
		...
		{
			AMTRS_DEBUG_LOG("LEA array can't not addressib;e NG");
			opecode = opecode_id::LOAD_INT;
		}
		else if (auto size = is_floating_point(_clz->name()); size)
		{
			AMTRS_DEBUG_LOG("LEA array can't not float NG");
			switch (size)
			{
				case 4 :	opecode = opecode_id::LOAD_FLOAT;	break;
				case 8 :	opecode = opecode_id::LOAD_DOUBLE;	break;
