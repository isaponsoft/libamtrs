...
#ifndef	__libamtrs__net__socket_traits_hpp
#define	__libamtrs__net__socket_traits_hpp
...
public:
	...
	static basic_socket<Traits> create_connect(const address& _addr)
	{
		if (_addr.empty())
		{
...
*/
	...
	// 
	int bind_inet(int _port)
	{
		struct sockaddr_in	addr;
	    addr.sin_family			= AF_INET;
	    addr.sin_addr.s_addr	= INADDR_ANY;
	    addr.sin_port			= htons(_port);
		return	bind((struct sockaddr*)&addr, sizeof(addr));
	}

	int bind_inet6(int _port)
	{
		struct sockaddr_in	addr;
	...
	{
		int	yes	= 1;
		setsockopt(get(), SOL_SOCKET, SO_REUSEADDR, (const char*)&yes, sizeof(yes));
		return	::bind(get(), _addr, _addrSize);
	}

	//! インターネットアドレスをバインドします。
	int bind(address& _addr)
	{
		return	::bind(get(), reinterpret_cast<const struct sockaddr*>(_addr.get()), sizeof(address::value_type));
	}


	int connect(char const* _host, int _port)
	{
		return	connect(address(_host, _port));
	}

	int connect(const address& _addr)
	{
		return	connect(reinterpret_cast<struct sockaddr const*>(_addr.get()), sizeof(address::value_type));
	}
	...
	{
		int	len	= (int)::recv(get(), (char*)_buf, (int)_buffSize, _flags);
		if (len == 0)
		{
			close();
		}
	...
	// shutdown(SHUT_RDWR)
	void shutdown(int _flags = SHUT_RDWR)
	{
		if (!empty())
		{
			::shutdown(mSocket, _flags);
		}
	}
