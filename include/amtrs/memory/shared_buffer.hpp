...
public:
	...
	shared_buffer(S const (&_ary)[N])
	{
		auto	inst	= instance_type::create(N);
		auto	src		= &_ary[0];	
		for (value_type* init = reinterpret_cast<value_type*>(inst + 1); inst->elmsize < N; ++(inst->elmsize))
		{
			new ((void*)&init[inst->elmsize]) value_type(*src++);
		}
		mInstance	= inst;
	...
	template<class R>
	shared_buffer(R&& _v, std::enable_if_t<can_copy_n<R, value_type>>** = 0)
	{
		auto	size	= _v.size();
		auto	inst	= instance_type::create(size);
		auto	src		= _v.begin();
		for (value_type* init = reinterpret_cast<value_type*>(inst + 1); inst->elmsize < size; ++(inst->elmsize))
		{
			new ((void*)&init[inst->elmsize]) value_type(*src++);
		}
		mInstance	= inst;
