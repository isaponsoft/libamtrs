...
public:
	...
	static ref<vkpipelinelayout> realize(shader_layout& _format, vkref<VkDevice> const& _device)
	{
		return	realizing<vkpipelinelayout>(_format, _device);
	}

	void initialize(shader_layout::instance_type* _owner, vkref<VkDevice> const& _device)
	{
		mOwner	= _owner;
		...
		for (auto& u : uniforms)
		{
			...
			bindings[i].stageFlags			= vkstage(u.stage);
			bindings[i].pImmutableSamplers	= nullptr;
			if (u.type == format_types::texture_value)
			{
				bindings[i].descriptorType		= VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
			}
			//AMTRS_INFO_LOG("Bindings : binding={}/{} desc={}, stage={}", (int)bindings[i].binding, (int)bindings.size(), std::to_string(bindings[i].descriptorType).c_str(), std::to_string((VkShaderStageFlagBits)bindings[i].stageFlags).c_str());
			++i;
		}
	...
	shader_layout::instance_type*	mOwner;
