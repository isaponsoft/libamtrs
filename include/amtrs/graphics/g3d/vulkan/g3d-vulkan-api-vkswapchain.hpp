...
struct	vkhandle_traits<VkSwapchainKHR>
{
	...
	uint64_t					timeout_nanosec		= 5 * 1000 * 1000;	// 5 milli sec
...
public:
	...
	VkResult next_image(vkref<VkSemaphore> const& _semaphore, vkref<VkFence> const& _fence = {})
	{
		...
		vklasterr	= vkAcquireNextImageKHR(std::get<VkDevice>(*this), get(), traits().timeout_nanosec, _semaphore.get(), _fence.get(), &traits().frameIndex);
		//_AMTRS_VKINVOKE_R(vkAcquireNextImageKHR(std::get<VkDevice>(*this), get(), traits().timeout_nanosec, _semaphore.get(), _fence.get(), &traits().frameIndex), vklasterr);
		return	vklasterr;
	}
