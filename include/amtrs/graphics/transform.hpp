...
public:
	...
	bool set_pivot(scale_type _position) noexcept { if (mPivot != _position) {mPivot = _position; update_matrix(); return true; } return false; }

	auto position() const noexcept -> position_type     { return mPosition; }
	bool set_position(position_type _position) noexcept { if (mPosition   != _position) { mPosition   = _position; update_matrix(); return true; } return false; }
	bool set_position_x(value_type _position) noexcept  { if (mPosition.x != _position) { mPosition.x = _position; update_matrix(); return true; } return false; }
	bool set_position_y(value_type _position) noexcept  { if (mPosition.y != _position) { mPosition.y = _position; update_matrix(); return true; } return false; }
	bool set_position_z(value_type _position) noexcept  { if (mPosition.z != _position) { mPosition.z = _position; update_matrix(); return true; } return false; }


	auto scale() const noexcept -> const scale_type& { return mScale; }
	bool set_scale(scale_type _scale) noexcept { if (mScale != _scale) { mScale = _scale; update_matrix(); return true; } return false; }

	auto rotate() const noexcept -> const rotate_type& { return mRotate; }
	bool set_rotate(rotate_type _rotate) noexcept { if (mRotate != _rotate) { mRotate = _rotate; update_matrix(); return true; } return false; }
	...
	void set_content_size(box_type _box) noexcept { mPosition = _box.position(); mSize = _box.size(); update_matrix(); }
