...
template<class CharT, class TraitsT>
std::basic_string_view<CharT, TraitsT> trim_first(std::basic_string_view<CharT, TraitsT> _string, std::basic_string_view<CharT, TraitsT> _delms)
{
	if (_string.empty())
	{
		return	_string;
...
template<class S, class Dem>
auto trim_first(S const& _string, Dem const& _dem)
{
	return	trim_first((std::string_view)_string, (std::string_view)_dem);
}
