...
//! Javaメソッドのラッパーインターフェースを作ります。
// ------------------------------------------------------------------------
#define	AMTRS_JAVA_DEFINE_METHOD_RENAMED(_name, _method, ...)	\
		template<class... Args>\
		auto _name(Args... _args)\
		{\
			overload_call<__VA_ARGS__>	func;\
			JNIEnv*						env	= get_jnienv();\
			handle_type					h	= ref_type::get();\
			if constexpr (std::is_same<handle_type, jobject>::value) { auto c = local_cls::make(env->GetObjectClass(h)); return func(_method, env, h, c.get(), typename jni_traits<Args>::argv_type(_args)...); }\
			if constexpr (std::is_same<handle_type, jclass >::value) {                                                   return func(_method, env, h,          typename jni_traits<Args>::argv_type(_args)...); }\
		}
...
struct	define_class : BaseClass
{
	...
	static jcls<class_type> find()
	{
		...
		JNIEnv*	env	= get_jnienv();
		//AMTRS_DEBUG_LOG("Find class \"%s\", jnienv=%p.", jni_traits<Class>::classname().c_str(), env);
		if (!gCls)
		{
			jclass	cls	= env->FindClass(jni_traits<Class>::classname().c_str());
			if (!cls)
			{
