...
struct	jni_traits<jintArray>
{
	...
	using					argv_type	= const jobj<jint[]>&;
...
template<>
struct	jni_traits<jint[]> : jni_traits<jintArray>
{
};
...
struct	jni_traits<jobj<T[]>>
{
	...
	using					argv_type	= const jobj<T[]>&;
	static constexpr auto	classname() noexcept { return jni_traits<T[]>::classname(); }
	static constexpr auto	signature() noexcept { return jni_traits<T[]>::signature(); }
};
