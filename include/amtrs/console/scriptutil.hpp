...
/*!
 * download() の結果を確認するための構造体
 */
struct	download_result
{
	uint16_t	status_code;	//!< HTTP ステータスコード
	std::string	savedname;		//!< ダウンロードしたファイルを保存した時のファイル名
	std::string	url;			//!< 最終的にダウンロードを行ったURL(Locationなどで移動した最終結果)

	operator bool () const noexcept { return status_code == 200; }
};
 ...
 * _savedname ダウンロードしたファイルを保存した時のファイル名を格納するバッファ。
 *
 * _savename
 *		ディレクトリの場合、_urlに応じたファイル名で保存します。
 *		ディレクトリ以外の場合はファイル名が指定されたものとしてファイルに書き込みます。
 */
