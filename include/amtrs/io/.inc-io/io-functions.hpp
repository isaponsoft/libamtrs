...
private:
	template<class C> constexpr static auto _read_rs(C& c) -> decltype(static_cast<size_type>(c.read(std::declval<char_type*>(), std::declval<size_type>())), std::true_type{});
	                  constexpr static auto _read_rs(... ) -> std::false_type;
	template<class C> constexpr static auto _read_gs(C& c) -> decltype(static_cast<size_type>(c.read(std::declval<char_type*>(), std::declval<size_type>()).gcount()), std::true_type{});
	                  constexpr static auto _read_gs(... ) -> std::false_type;
	template<class C> constexpr static auto _read_only(C& c) -> decltype(c.read(std::declval<char_type*>(), std::declval<size_type>()), std::true_type{});
	                  constexpr static auto _read_only(... ) -> std::false_type;


	template<class C> constexpr static auto _write_rs(C& c) -> decltype(static_cast<size_type>(c.write(std::declval<char_type*>(), std::declval<size_type>())), std::true_type{});
	                  constexpr static auto _write_rs(... ) -> std::false_type;
	template<class C> constexpr static auto _write_ps(C& c) -> decltype(static_cast<size_type>(c.write(std::declval<char_type*>(), std::declval<size_type>()).pcount()), std::true_type{});
	                  constexpr static auto _write_ps(... ) -> std::false_type;
	template<class C> constexpr static auto _write_only(C& c) -> decltype(c.write(std::declval<char_type*>(), std::declval<size_type>()), std::true_type{});
	                  constexpr static auto _write_only(... ) -> std::false_type;


	template<class C> constexpr static auto _rdstate(C& c) -> decltype(static_cast<std::ios_base::iostate>(c.rdstate()), std::true_type{});
	                  constexpr static auto _rdstate(... ) -> std::false_type;


	template<class C> constexpr static auto _size(C& c) -> decltype(static_cast<size_type>(c.size()), std::true_type{});
	                  constexpr static auto _size(... ) -> std::false_type;

	template<class C> constexpr static auto _seek(C& c) -> decltype(c.seek(0, std::ios::beg), std::true_type{});
	                  constexpr static auto _seek(... ) -> std::false_type;
	template<class C> constexpr static auto _tell(C& c) -> decltype(static_cast<size_type>(c.tell()), std::true_type{});
	                  constexpr static auto _tell(... ) -> std::false_type;
	template<class C> constexpr static auto _seekg(C& c) -> decltype(c.seekg(0, std::ios::beg), std::true_type{});
	                  constexpr static auto _seekg(... ) -> std::false_type;
	template<class C> constexpr static auto _tellg(C& c) -> decltype(static_cast<size_type>(c.tellg()), std::true_type{});
	                  constexpr static auto _tellg(... ) -> std::false_type;
	template<class C> constexpr static auto _seekp(C& c) -> decltype(c.seekp(0, std::ios::beg), std::true_type{});
	                  constexpr static auto _seekp(... ) -> std::false_type;
	template<class C> constexpr static auto _tellp(C& c) -> decltype(static_cast<size_type>(c.tellp()), std::true_type{});
	                  constexpr static auto _tellp(... ) -> std::false_type;
public:
	...
	static constexpr decltype(_read_only(std::declval<T&>()))	has_read_only			= decltype(_read_only(std::declval<T&>())){};
	...
	static constexpr decltype(_rdstate(std::declval<T&>()))		has_rdstate				= decltype(_rdstate(std::declval<T&>())){};
...
AMTRS_IMPLEMENTS_END(iotest)
using namespace AMTRS_IMPLEMENTS(iotest);
...
auto read(CharT* _buff, In& _in, size_t _size)
{
	...
	{
//		static_assert(0, "Unkown read function.");
	}
}
...
{
	auto	s	= read(&_buff, _in, sizeof(T));
	if (s != sizeof(T))
	{
		return	false;
	}
...
auto write(Out& _out, CharT const* _data, size_t _size)
{
	...
	{
//		static_assert(0, "Unkown write function.");
	}
}
