...
public:
	...
	using	handler_func	= bool(window& _sender, event_data const& _event);


	virtual ~window() { if (*window_instance() == this) { *window_instance() = nullptr; } }


	static ref<window> create(std::string _title, size_type _size, std::function<handler_func> _proc);

	static ref<window> create(std::string _title, size_type _size) { return create(std::move(_title), _size, {}); }
...
protected:
	...
	bool fire_activate(bool _active);
	bool fire_created();
	bool fire_destroyed();
	bool fire_focus(bool _focus);
	bool fire_input(input_event* _input);
	bool fire_sizechanged(size_type _size);
	bool fire_repaint(rect<int> _area);
